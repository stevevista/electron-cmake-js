...
const execResult = execFileSync(electron, [path.resolve(__dirname, 'electron-abi.js')])
const VERSINOS = execResult.toString().split(',').map(x => x.trim())
const [ABI, ELECTRON_VERSION] = VERSINOS
...
CMake.prototype.getConfigureCommand = async function () {
    ...
    // CMake.js watermark
    D.push({"ELECTRON_CMAKE_SUBMODULE_PATH": path.resolve(__dirname, 'node-addon.cmake')});
    D.push({"ELECTRON_APP_ROOT_PATH": path.resolve(path.dirname(require.resolve('electron')), '../..').replace(/\\/g, '/') });

    D.push({"NODE_ABI": ABI});
    D.push({"ELECTRON_VERSION": ELECTRON_VERSION});
    ...
    D.push({"DELAY_LOAD_SRC": srcPaths.join(";")});
